  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card>
          <p className="text-sm text-gray-500">프로젝트 수</p>
          <p className="text-3xl font-bold text-gray-900 mt-1">{(projectCount as number | null) ?? 0}</p>
        </Card>
        <Card>
          <p className="text-sm text-gray-500">이번달 API 호출</p>
          <p className="text-3xl font-bold text-gray-900 mt-1">{(monthlyApiCount as number | null) ?? 0}</p>
        </Card>
        <Card>
      ...
      <Card title="최근 쿼리 로그">
        {typedLogs.length === 0 ? (
          <p className="text-sm text-gray-500">쿼리 기록이 없습니다.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-2 pr-4 font-medium text-gray-600">질문</th>
                  <th className="text-left py-2 pr-4 font-medium text-gray-600">상태</th>
                  <th className="text-left py-2 pr-4 font-medium text-gray-600">신뢰도</th>
                  <th className="text-left py-2 font-medium text-gray-600">일시</th>
                </tr>
              </thead>
              <tbody>
                {typedLogs.map((log) => (
